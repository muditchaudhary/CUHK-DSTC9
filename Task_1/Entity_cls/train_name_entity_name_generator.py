...
def main():
	with open('train_mudit/logs.json','r') as f:
		logs=json.load(f)
	with open('domain_cls_full_trainingData.json','r') as f:
		domain_preds=json.load(f)
	with open('train_mudit/labels.json','r') as f:
		labels=json.load(f)
	with open('train_mudit/knowledge.json','r') as f:
		knowledges=json.load(f)
	preds=[]
	logs=logs[39905:]
	labels=labels[39905:]
	domain_preds = domain_preds[39905:]
	...
	# exit()
	num=0
	for log , domain_pred,label in tqdm(zip(logs,domain_preds,labels)):
