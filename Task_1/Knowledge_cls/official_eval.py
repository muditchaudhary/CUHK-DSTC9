...
#         print(example)
        ...
        # gt_labels.append(example)
    # for label, pred in zip(gt_labels,preds):
    #     if pred['labels'][0].lower()==label['choices'][0].lower():
    #         accuracy+=1
    #     if label['choice_types'][label['choices'].index(pred['labels'][0])]==label['choice_types'][0]:
    #         knowledge_detection_accuracy+=1
    #         if label['choice_types'][0]==True:
    #             tp+=1
    #         else:
    #             tn+=1
    #     else:
    #         if label['choice_types'][0]==False:
    #             fp+=1
    #         else:
    #             fn+=1
    print(preds)
...
#         print(example['choice_types'])
#         print(example['choices'].index(pred['labels'][0]))
#         print(example['choice_types'][example['choices'].index(pred['labels'][0])])
#         print(
#         exit()
#         pred_types.append(example['choice_types'][example['choices'].index(pred['labels'][0])])
#         pred_answers.append(pred['labels'][0])
...
#         exit()
    ...
    # eval_result = { "checkpoint": model,"accuracy": accuracy/len(preds), "knowledge_detection_accuracy": knowledge_detection_accuracy/len(preds),
    #                "recall": recall,
    #                "precision": precision, "f1": f1,'tp':tp,'fp':fp,'tn':tn,'fn':fn}
    # with open(os.path.join(args.output_dir,'official_evaluate_result.txt'),'a') as f:
