...
def write_selection_preds(dataset_walker, output_file, data_infos, sorted_pred_ids, all_preds, topk=5):
    # Flatten the data_infos
    data_infos = [
        {
            "dialog_id": info["dialog_ids"][i],
    ...
    for info, sorted_pred_id, logits in zip(data_infos, sorted_pred_ids, all_preds):
        dialog_id = info["dialog_id"]
        candidate_keys = info["candidate_keys"]
        count += 1
        snippets = []
        for pred_id in sorted_pred_id: 
        # for pred_id in sorted_pred_id[:topk]: 
            selected_cand = candidate_keys[pred_id]
            domain, entity_id = selected_cand.split("__")
            snippet = {
                'logits': logits[0][pred_id].item(),
                "domain": domain,
...
def truncate_sequences(sequences, max_length):
    # sum length of all sequence in sequences
    words_to_cut = sum(list(map(len, sequences))) - max_length
    # no need to truncate
    if words_to_cut <= 0:
        return sequences
    # each time drop the 1st (fartest) sequence (sequences[0]), as the later sequence the more information it provides
