...
def train(args, train_dataset, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, run_batch_fn_train, run_batch_fn_eval) -> Tuple[int, float]:
    ...
    print("======================= INFO =====================")
    print("args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu) = ", args.train_batch_size)
    print("length of train_dataloader = ",len(train_dataloader))
    print("t_total = len(train_dataloader) // args.gradient_accumulation_steps * args.num_train_epochs = ", t_total)
...
def evaluate(args, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, run_batch_fn, desc="") -> Dict:
    ...
    print("length of eval_dataloader = ",len(eval_dataloader))
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating", disable=args.local_rank not in [-1, 0]):
        ...
        nb_eval_steps += 1
        if count % 100 == 0: 
            print(type(mc_logits))
            print(mc_logits[0].size())
            print(mc_logits[0][0])

    print("lenth of all_preds = ", len(all_preds),'\n\n')
    ...
    if args.task.lower() == "generation":
        ...
        if args.output_file:
            ...
            write_selection_preds(eval_dataset.dataset_walker, args.output_file, data_infos, sorted_pred_ids, all_preds, topk=5)
...
def main():
    ...
    if args.eval_only:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path) ### model_name_or_path: params.json ==> bert
    ...
    ## not exceed if set eval_only
    if not args.eval_only:
        train_dataset = dataset_class(dataset_args, tokenizer, split_type="train")
        ## ？？？ validation set use labels.json
        ## use knowledge in labels.json to generate negative samples
    ...
    result = {}
    if args.local_rank in [-1, 0]:
        print("start KnowledgeSelectionDataset ...\n")
