...
#         labels=labels.long()
#     )
#     loss, logits, hidden_states, attentions = model_outputs[0], model_outputs[1], model_outputs[2], model_outputs[3]
#     return loss, logits, attentions, labels

def run_batch_selection_train(args, model, batch):
    batch = tuple(input_tensor.to(args.device) for input_tensor in batch if isinstance(input_tensor, torch.Tensor))
    input_ids, token_type_ids, lm_labels, labels, attention_mask = batch
    model_outputs = model(
        input_ids=input_ids, token_type_ids=token_type_ids,
        attention_mask=attention_mask,
        labels=labels.long()
    )
    loss, scores = model_outputs[0], model_outputs[1]
...
def run_batch_selection_eval(args, model, batch):
    ...
    all_logits = []
    # print("*********************")
    # print("input_ids.size(1) = ", input_ids.size(1)) # 2900 = knowledge number
    # print("size of input_ids = ", input_ids.size())
    # print("candidates_per_forward = ", candidates_per_forward) # 32
    # print("max_candidates_per_forward_eval = ", args.max_candidates_per_forward_eval) # 16
    for index in range(0, input_ids.size(1), candidates_per_forward):
        model_outputs = model(
            input_ids=input_ids[0, index:index+candidates_per_forward].unsqueeze(1),
            token_type_ids=token_type_ids[0, index:index+candidates_per_forward].unsqueeze(1),
    ...
    all_logits = torch.cat(all_logits, dim=0).unsqueeze(0)
    # print("size of all_logits: ",all_logits.size())
    # print("input_ids.size(1) / candidates_per_forward = ", input_ids.size(1)/candidates_per_forward)
    return torch.tensor(0.0), torch.tensor([]), all_logits, labels
