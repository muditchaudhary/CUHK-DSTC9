...
class QuestionReconstructDataset(BaseDataset):
    def __init__(self, args, tokenizer, split_type, labels=True, labels_file=None):
        super(QuestionReconstructDataset, self).__init__(args, tokenizer, split_type, labels, labels_file)
    ...
    def build_input_from_segments(self, knowledge, history, with_eos=True):
        """ Build a sequence of input from 2 segments: knowledge, history """
        instance = {}

        sequence = [[self.bos] + knowledge] + history[:-1] + [history[-1] + ([self.eos] if with_eos else [])]
        sequence_with_speaker = [
            [self.speaker1 if (len(sequence) - i) % 2 == 0 else self.speaker2] + s
            for i, s in enumerate(sequence[1:])
        ]
            ...
            if target:
                ...
                if self.split_type == "train" and self.args.negative_sample_method == "oracle":
                    ...
                    if len(knowledge_candidates) < self.args.n_candidates:
                        continue
                used_knowledge = self.snippets[knowledge_key]   
                knowledge_candidates_wrong = [cand for cand in self.snippets.keys() if cand.startswith(prefix) and not cand.startswith(knowledge_key)]
                ###
                k_knowledge=random.sample(knowledge_candidates_wrong, self.args.k_knowledge-1)
                for i in range(len(k_knowledge)):
                    used_knowledge = self.snippets[k_knowledge[i]] + [self.knowledge_sep] + used_knowledge
                ###          
                used_knowledge = used_knowledge[:self.args.knowledge_max_tokens]
            else:
                knowledge_candidates = None
    ...
    def _create_examples(self):
        ...
        for dialog in tqdm(self.dialogs, disable=self.args.local_rank not in [-1, 0]):
            ...
            if target:
                ...
                ###
                if label is not None:
                    for i in range(1, self.args.k_knowledge):
                        cand_knowledge = label["knowledge"][i]
                        cand_knowledge_key = "{}__{}__{}".format(cand_knowledge["domain"], cand_knowledge["entity_id"], cand_knowledge["doc_id"])
                        used_knowledge = self.snippets[cand_knowledge_key] + [self.knowledge_sep] + used_knowledge
                ###          
                used_knowledge = used_knowledge[:self.args.knowledge_max_tokens]
            else:
                knowledge_candidates = None
...
class KnowledgeSelectionDataset(BaseDataset):
    ...
    def _knowledge_to_string(self, doc, name=""):
        join_str = " %d " % self.tokenizer.convert_tokens_to_ids(self.knowledge_sep_token)
